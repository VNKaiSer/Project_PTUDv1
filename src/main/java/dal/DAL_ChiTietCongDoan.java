...
public class DAL_ChiTietCongDoan {
    ...
    /**
     * Hàm kiểm tra sản phẩm đã chia hoàn thành công đoạn hay chưa
     * @param maSP
     * @return
     * @throws SQLException
     */
    public int checkPhanCD(String maSP) throws SQLException {
        String sql = "{?= call [dbo].[checkChiaCongDoan](?)}";
        ConnectDB.getInstance().connect();
    ...
    /**
     * Hàm được sử dụng khi một sản phẩm đã hoàn thành công đoạn
     * @param maCongDoan
     * @throws SQLException
     */
    public void insertTrangThaiDaChiaCD(String maCongDoan) throws SQLException {
        // gọi kết nối
        ConnectDB.getInstance().connect();
        String sql = "INSERT INTO TrangThaiCongDoan\n" +
                     "VALUES (?,?)";
        PreparedStatement ppsm = ConnectDB.getConnection().prepareStatement(sql);
        ppsm.setString(1, maCongDoan);
        ppsm.setBoolean(2, true);
        ppsm.execute();
        // đóng kết nối
        ConnectDB.getConnection().close();
    }
